...
#                 print(this_loss)
        ...
        print("train, ep: {} loss: {:.2f} accs: {:.2f}".format(ep, 
                                                            np.mean(train_loss), 
                                                            np.mean(train_accs*train_ratios)))
        ...
        with torch.no_grad():
            ...
            val_acc = np.mean( np.array(val_accs) * val_ratios )
...
def main():
    ...
    train_data = fetch_data_as_frame(fetching='../../data/short_sequences/gi31_raw_data.fasta', file_type='fasta')
    
    so_connected = soCo(client_token="YOUR_SIGOPT_TOKEN")
    ...
    exec_report_file = 'pn_sigopt_exec_report_p%s2.txt' % (''.join((str(x) for x in test_partitions)))
    ...
    if len(so_experimental) == 0: 
        for part in test_partitions:
            so_experimental.append(so_connected.experiments().create(
                name='NetGPI - Max heuristic - TP: %d' % (part),
                project='netgpi',
                metrics=[
                    { 'name': 'heuristic',
                    'objective': 'maximize' } ],
    ...
    for experiment_ind, test_partition, in enumerate(test_partitions):
        """ One experiment for each test partition """
        experiment = so_experimental[experiment_ind]
        for on in range(so_budget):
            ...
            if time.time() - total_runtime >= 82800:
                ...
                return
            ## Get a suggestion
            suggestion = so_connected.experiments(experiment.id).suggestions().create()
            ...
            fold_measures = []
            fold_models = {'models':[], 'measure': 0, 'params': params}
            for val_partition in (p for p in partitions if p!=test_partition):
                ...
                t1 = time.time() - t0
                fold_measures.append(m['heuristic'])
                fold_models['models'].append(m)
            ...
            fold_models['measure'] = float(np.mean(fold_measures))
            so_connected.experiments(experiment.id).observations().create(
                suggestion=suggestion.id,
                value=fold_models['measure'],
                value_stddev=0.0125
            )
            ## store model if better:
            model_filepath = "../../picklejar/model_ensemble_p%d.pt" % (params['test-partition'])
            if not path.exists(model_filepath):
                torch.save(fold_models, model_filepath)
            else:
                saved_model = torch.load(model_filepath)
                if saved_model['measure'] < fold_models['measure']:
                    torch.save(fold_models, model_filepath)
